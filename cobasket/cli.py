...
def screen_cmd():
    ...
    parser.add_argument(
        "--watchlist-out",
        help="Optional JSON path for the top ranked baskets as a BasketWatchlist",
    )
    ...
    if confirmed:
        ...
        if args.watchlist_out:
            _save_ranked_watchlist(
                results,
                args.watchlist_out,
                args.top_n,
                name=f"Cobasket correlation screen ({args.period})",
            )
...
def pca_screen_cmd():
    ...
    print(f"\nSaved {scree_path}")
    print("  Look at this first: the elbow indicates how many PCs carry substantial structure.")

    candidate_baskets, linkage_matrix = cluster_by_loadings(
        loadings, args.n_components_for_clustering, args.distance_threshold
    )
    basket_labels = {}
    for cluster_id, members in enumerate(candidate_baskets):
        for ticker in members:
            basket_labels[ticker] = cluster_id
    ...
    dendro_path = plot_dendrogram(
        linkage_matrix,
        loadings.index,
        path=f"{args.plot_dir}/cluster_dendrogram.png",
        distance_threshold=args.distance_threshold,
    )
    ...
    if confirmed:
        ...
        if args.watchlist_out:
            _save_ranked_watchlist(
                ...
                args.watchlist_out,
                args.top_n,
                name=f"Cobasket PCA screen ({args.period})",
            )
    elif args.watchlist_out:
        print(f"\nNo confirmed baskets; watchlist {args.watchlist_out} was not created.")
